...
export const SHIPPING_OPTIONS: ShippingOption[] = [
  {
    id: "standard",
    labelKey: "shipping.standard",
    etaKey: "shipping.standardEta",
    priceUsd: 3,
    freeOverUsd: 150,
  },
  {
    id: "express",
    labelKey: "shipping.express",
    etaKey: "shipping.expressEta",
    priceUsd: 7,
  },
  {
    id: "pickup",
    labelKey: "shipping.pickup",
    etaKey: "shipping.pickupEta",
    priceUsd: 0,
  },
];
...
/**
 * Returns a translation key (and params) for why the code was refused, or `null` when it is
 * usable. The data layer stays locale-free; PromoField turns the key into text.
 */
export function promoRejection(
  ...
  if (!trimmed) return { key: "promo.empty" };
  const promo = findPromo(trimmed);
  if (!promo) return { key: "promo.invalid", params: { code: trimmed.toUpperCase() } };
  if (promo.minSubtotalUsd && subtotalUsd < promo.minSubtotalUsd) {
    return {
      key: "promo.minSubtotal",
      params: { code: promo.code, amount: promo.minSubtotalUsd },
    };
  }
  return null;
}
